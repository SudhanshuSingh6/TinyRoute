...
public class AuthController {
    ...
    private void enforceAuthRateLimit(HttpServletRequest request) {
        try {
            ...
            String key = AuthController.LOGIN_KEY_PREFIX + ipHash;
            Bucket bucket = rateLimitService.resolveBucket(key, RateLimitPlan.AUTH);
            ...
            if (!probe.isConsumed()) {
                ...
                throw new RateLimitExceededException("Too many " + "login" + " attempts. Please try again in " + retryAfter + " seconds.", retryAfter);
            }
